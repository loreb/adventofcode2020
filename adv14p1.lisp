...
(defun apply-mask(mask value)
  (let ((bits '()) (ret 0))
    (LOOP for ch in (reverse (coerce (bitmask-mask mask) 'list)) DO
          (cond ((equal BITX ch) (setf bits (cons (logand 1 value) bits)))
                ((equal BIT0 ch) (setf bits (cons 0 bits)))
                ((equal BIT1 ch) (setf bits (cons 1 bits)))
                ('else (error "nope")))
          (setf value (ash value -1))
          )
    (LOOP for bit in bits DO
          (setf ret (+ ret ret bit))
          )
    ret
  )
  )
  ...
  (let ((mem (make-hash-table :test #'equalp))
    ...
    (dolist (ins instructions)
      (cond ((setmask-p ins) (setf bitmask (setmask-mask ins)))
            ((memorywrite-p ins) (setf (gethash (memorywrite-addr ins) mem)
                                       (apply-mask bitmask (memorywrite-value ins))))
            ('else (error ins))
            )
      )
    mem
...
(let ((n (apply-mask (decode-mask "XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X") 11)))
  (when (not (equal n 73))
    (error (format nil "got~A instead of 73" n))
    ))
