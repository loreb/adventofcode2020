...
sub invalid_permutation {
    my $ticketsref = shift or die "!";
    my @perm = @_;
    foreach my $ticket(@$ticketsref) {
        my $i = 0;
        foreach my $field(@perm) {
            my $r = $rule{$field};
            my $value = $ticket->[$i];
            $i++;
            next if value_follows_rule($value, $r);
            return 1;
        }
    }
    return 0;
...
my $perm = new List::Permutor (keys %rule);
my $found = 0;
my @validperm;
while (my @set = $perm->next) {
    $dbg->("permutation: ", join(' ', @set));
    next if invalid_permutation(\@tickets, @set);
    $found = 1;
    @validperm = @set;
    last;
}
die "nothing valid?" unless $found;
$dbg->("valid permutation: ", join(' ', @validperm));
my %fieldidx;
for (my $i = 0; $i < arraylen(@validperm); $i++) {
    $fieldidx{$validperm[$i]} = $i;
}
